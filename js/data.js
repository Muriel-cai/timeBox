(function (factory) {
  ...
  $.fn.extend({
    date: function (options) {
      ...
      var option = {
        beginYear: options.beginYear || 2010, //可不填，开始时间
        endYear: options.endYear || 2088, //可不填，结束年份不会小于当前年份
        startTime: options.startTime || 9, //可不填，开始时间不能早于9点
        endTime: options.endTime || 21, //可不填，结束时间不能早于9点
        type: options.type || "YYYY-MM-DD",
        limitTime: options.limitTime || false, //限制选择时间 today 今天之前的时间不可选 tomorrow 明天之前的不可选
        location: options.location || null, //before 跳转至之前选择的时间，如果为空则跳转至当前时间
      ...
      var d = new Date(),
        ...
        dateFormat = [
          ...
          //年月日 时
          ["YYYY-MM-DD hh", "YY-MM-DD hh", "YYYY/MM/DD hh", "YY/MM/DD hh"],
          //时分秒 时分
          ["hh:mm:ss", "hh:mm"]
        ],
      ...
      createDate = {
        ...
        resetActive: function (el) {
          ...
          if (option.location == 'before' && date) {
            d = new Date(date);
          ...
          if (option.limitTime == 'tomorrow' && !option.location) {
      ...
      $(this).on('click', function () {
        ...
        if ($(this).is(':visible')) {
          var y = '' + option.beginYear + option.endYear;
          if (prevY != y) {
            createDate.y(option.beginYear, option.endYear);
            prevY = y;
          }
          // console.log(this, dateFormat, "+++++++11111111111111111111111111++++++++++++++++++++++++", )
          if (dateFormat[0].indexOf(option.type) != -1) { //年月日 时分秒
            elTit.children().show();
            elBg.children().show();
            el.attr('class', '');
          } else if (dateFormat[1].indexOf(option.type) != -1) { //年月日 时分
            elTit.children().show().end().children(':gt(4)').hide();
            elBg.children().show().end().children(':gt(4)').hide();
            el.attr('class', 'five');
          } else if (dateFormat[2].indexOf(option.type) != -1) { //年月日
            elTit.children().show().end().children(':gt(2)').hide();
            elBg.children().show().end().children(':gt(2)').hide();
            el.attr('class', 'three');
          } else if (dateFormat[3].indexOf(option.type) != -1) { //年月
            elTit.children().show().end().children(':gt(1)').hide();
            elBg.children().show().end().children(':gt(1)').hide();
            el.attr('class', 'two');
          } else if (dateFormat[4].indexOf(option.type) != -1) { //年月日 时
            elTit.children().show().end().children(':gt(3)').hide();
            elBg.children().show().end().children(':gt(3)').hide();
            el.attr('class', 'four');
          } else if (dateFormat[5].indexOf(option.type) == 0) { //时分秒
            elTit.children().show().end().children(':lt(3)').hide();
            elBg.children().show().end().children(':lt(3)').hide();
            el.attr('class', 'three hms');
          } else if (dateFormat[5].indexOf(option.type) == 1) { //时分
      ...
      $('#d-confirm').bind('click', function () {
        ...
        var y = $('#d-year .active').text(),
          ...
          str,
          that = $(this),
          index = dateFormat[0].indexOf(option.type),
          index1 = dateFormat[1].indexOf(option.type),
          index2 = dateFormat[2].indexOf(option.type),
          index3 = dateFormat[3].indexOf(option.type),
          index4 = dateFormat[4].indexOf(option.type),
          index5 = dateFormat[5].indexOf(option.type);
        // if(opt.startTime > parseFloat(h) || parseFloat(h) > opt.endTime){
        //     $('.toast').show();
        //     $('.toast').html("取还车时间不能早于"+ opt.startTime + "点 </br>取还车时间不能早于" + opt.endTime +"点");               
        //     setTimeout(hide, 2000);
        //     // console.log($('.toast'),"取车时间不能早于9");
        //     return false;
        // } 
        // 回调函数
        ...
        if (index != -1) { //年月日 时分秒
          ...
          switch (index1) {
            case 0:
              str = y + '-' + m + '-' + d + ' ' + h + ':' + min;
              //   timefm = ;
              //   console.log(str,timefm,s,"ces")
              break;
            case 1:
              str = y.substring(2) + '-' + m + '-' + d + '  ' + h + ':' + min;
              // console.log(str,y.substring(2))
              break;
            case 2:
              str = y + '/' + m + '/' + d + ' ' + h + ':' + min;
              break;
        ...
        } else if (index5 == 0) { //时分秒
          str = h + ':' + min + ':' + s;
        } else if (index5 == 1) { //时分
          str = h + ':' + min;
        }

        if (option.limitTime == 'today') {
          // console.log(1456222555)
          var d = new Date(),
        ...
        } else if (option.limitTime == 'tomorrow') {
          var d = new Date(),
        ...
        // console.log(str, "fun+++++++++++++++");
        var fun = option.callback;
        fun(str);
        if (option.location) {
          $(this).data('fullDate', y + '-' + m + '-' + d + ' ' + h + ':' + min + ':' + s);
        }
        createDate.show(false);
        createDate.toNow(true);
      });
